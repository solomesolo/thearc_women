-- ARC enriched recommendation schema — CMO fixed version
-- Fixes broad/incorrect biomarker placement by adding curated check-specific biomarker mappings, signal templates, and pathway persistence tables.
...
CREATE TABLE check_biomarker_curation_notes (
  check_key TEXT,
  principle TEXT,
  included_logic TEXT,
  excluded_logic TEXT,
  max_core_biomarkers INTEGER,
  created_at TEXT
);
...
SELECT
  ...
  s.signal_label,
  t.check_key,
  c.display_name AS check_name,
  ct.biomarker_id,
  ct.biomarker_name,
  ct.included_category,
  ct.priority_tier,
  ct.why_included,
  t.template AS why_signal_matters,
  rc.template AS short_summary_template,
  c.what_can_wait_template,
  c.max_core_biomarkers,
  slot.max_core
FROM answer_signal_explanations s
JOIN survey_answer_options r ON r.field_key=s.field_key AND r.answer_option=s.answer_option AND ifnull(r.test_bundle,'')=ifnull(s.test_bundle,'')
JOIN signal_check_explanation_templates t ON t.signal_key=s.signal_key
JOIN canonical_check_config c ON c.check_key=t.check_key
JOIN check_included_tests ct ON ct.check_key=t.check_key
LEFT JOIN recommendation_content_templates rc ON rc.check_key=t.check_key AND rc.content_type='summary'
LEFT JOIN canonical_check_biomarker_slots slot ON slot.check_key=t.check_key;
...
-- 1. Use v_recommendation_content_source for UI-facing recommendation content, not raw answer_option_biomarker_rules.
-- 2. check_included_tests is now curated by check_key; core_now contains only aligned 3-4 biomarkers/screenings per check.
-- 3. Apply the global cap of max 10 core_now blood biomarkers in the service layer when building a user's current_month plan.
-- 4. Use signal_check_explanation_templates for personalized whyRecommendedForYou copy.
-- 5. user_pathway_biomarker_selection and user_pathway_schedule persist the final generated user plan.


-- =========================================================
-- Personalized WHY THIS MATTERS rules
-- =========================================================
