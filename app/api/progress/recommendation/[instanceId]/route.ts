...
import { getCurrentProgressForInstance } from "@/lib/progress-engine/progressService";
...
export async function GET(_request: Request, context: { params: Promise<{ instanceId: string }> }) {
  ...
  try {
    const result = await getCurrentProgressForInstance({
      userEmail: session.user.email,
      recommendationInstanceId: recommendation_instance_id,
    });
    return Response.json(result);
  } catch (err: any) {
    return new Response(JSON.stringify({ error: err?.message ?? "Failed to fetch progress" }), {
      status: 500,
