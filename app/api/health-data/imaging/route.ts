/**
 * GET /api/health-data/imaging
 *
 * Imaging records for the authenticated user.
 * Supports pagination:
 *   ?page=1&pageSize=20
 */
...
export async function GET(req: NextRequest) {
  ...
  const [total, rows] = await Promise.all([
    prisma.imagingRecord.count({ where }),
    prisma.imagingRecord.findMany({
      where,
...
/**
 * DELETE /api/health-data/imaging
 *
 * Body:
 *   { documentIds: string[] }
 *
 * Deletes selected imaging records for the authenticated user only.
 */
export async function DELETE(req: NextRequest) {
  ...
  const prisma = getPrisma();
  const result = await prisma.imagingRecord.deleteMany({
    where: {
