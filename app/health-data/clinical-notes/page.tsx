...
export default function ClinicalNotesPage() {
  const [data, setData] = useState<{ records: ClinicalNoteRecord[]; pagination: Pagination } | null>(null);
  ...
  const fetchData = useCallback(async (pg: number) => {
    ...
    try {
      ...
      const res = await fetch(`/api/health-data/clinical-notes?${params}`);
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <p className="mt-1.5 text-[14px] text-[var(--text-secondary)]">
              Letters and notes extracted from your documents (non-lab, non-imaging).
            </p>
          </div>
          ...
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-20 text-center">
              ...
              <p className="mt-2 text-[13px] leading-[1.6] text-[var(--text-secondary)] max-w-xs">
                Upload a consultation letter or clinical note to add it here.
              </p>
              <Link
                href="/upload"
              ...
              <div className="space-y-3">
                {data!.records.map((r) => (
                  <div key={r.documentId} className="rounded-[18px] border border-black/[0.07] bg-white px-5 py-5">
                      ...
                      <div className="flex items-center gap-3">
                        <button
                          ...
                          onClick={async () => {
                            ...
                            try {
                              const res = await fetch(`/api/health-data/clinical-notes/${r.documentId}/preview`);
                              if (!res.ok) return;
                        ...
                        <button
                          ...
                          disabled={busyId === r.documentId}
                          onClick={async () => {
                            if (!confirm("Delete this clinical note record? This cannot be undone.")) return;
                            setBusyId(r.documentId);
                            try {
                              const res = await fetch("/api/health-data/clinical-notes", {
                                method: "DELETE",
                    ...
                    <Section title="Notes" text={r.rawNotes} />
